...
class AppSubscription_item extends StatefulWidget {
  const AppSubscription_item({Key? key}) : super(key: key);

  @override
  State<AppSubscription_item> createState() => _AppSubscription_itemState();
}

class _AppSubscription_itemState extends State<AppSubscription_item> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        child: StreamBuilder<QuerySnapshot>(
            ...
            child: StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance.collection('AppSubscription').snapshots(),
          builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return Center(
                child: CircularProgressIndicator(),
              ...
              return Padding(
                ...
                child: GridView(
                  ...
                  children: snapshot.data!.docs
                      .map((detail) => appitemCard(() {
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (_) => appDetailPage(detail)));
                          }, detail))
                      .toList(),
                ),
              );
